...
- **Registro de peers** por nombre — sin necesidad de recordar IPs manualmente
...
| `POST` | `/peers` | Registrar peer (`?name=&ip=&port=`) |
...
| `POST` | `/enviar/broadcast` | Enviar a todos los peers concurrentemente |

La documentación interactiva de la API está disponible en `http://localhost:5003/docs`.

## Comunicación entre nodos

Para que dos nodos se comuniquen deben estar en la misma red. Cada uno registra al otro como peer con su IP local.
